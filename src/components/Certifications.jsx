...
function CertCard({ title, date, badges, verifyUrl }){
  return (
    <motion.div whileHover={{y:-6}} className="card hover:border-cyan-400/50 transition-all">
      ...
      <div className="mt-6">
        <a href={verifyUrl} target="_blank" rel="noreferrer" className="px-4 py-2 inline-block rounded-md border border-cyan-400 text-cyan-400 hover:bg-cyan-400 hover:text-black transition">Verify Credential</a>
      </div>
    </motion.div>
  )
}
    ...
    <section id="certifications" className="py-20">
      <div className="container">
        ...
        <div className="mt-8 grid sm:grid-cols-3 gap-6">
          {items.map(i => <CertCard key={i.title} {...i} verifyUrl={folderUrl} />)}
        </div>
      </div>
    </section>
  )
