import React from 'react'
import { motion } from 'framer-motion'

export default function Hero(){
  return (
    <section id="hero" className="py-24">
      <div className="container flex items-center gap-12">
        <div className="flex-1">
          <div className="text-cyan-400 font-medium">Hello !!!</div>
          <h1 className="mt-4 text-6xl font-extrabold font-display">I'm <span className="text-cyan-400">Santhosh</span> Vemuluri</h1>
          <div className="inline-block mt-4 px-3 py-1 rounded-full bg-slate-900/40 border border-slate-700 text-xs font-semibold tracking-widest">ASPIRING SOFTWARE ENGINEER</div>
          <p className="mt-6 text-slate-300 max-w-xl">B.Tech student focused on building scalable digital solutions, data-driven dashboards, and secure full-stack applications with embedded systems experience.</p>
          <div className="mt-6 flex gap-4">
            <a href="#projects" className="bg-cyanAccent px-6 py-3 rounded-md font-bold text-black uppercase">VIEW PROJECTS</a>
            <a href="#contact" className="border border-slate-700 px-6 py-3 rounded-md text-white uppercase hover:border-cyan-400">GET IN TOUCH</a>
          </div>
        </div>
        <motion.div className="w-72 rounded-2xl bg-slate-900/50 p-4 shadow-xl" initial={{y:20, opacity:0}} animate={{y:0, opacity:1}} transition={{duration:0.6}}>
          <img src="/assets/drive/hero_right.jpg" alt="portrait" className="w-full h-auto rounded-lg object-cover" />
        </motion.div>
      </div>
    </section>
  )
}
